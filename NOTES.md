...
Eight experiments about what an agent ran, what it changed, and what another person can check. Tools, inputs, and results are Git objects, so we can preserve the exact work, share it, and rerun it.
...
The tests still determine what passing means. Pinning them makes that choice visible and stops the submission from quietly changing it.
...
The decisive step is the parent's fixed destination: the child can propose files, but cannot choose where they are applied. The read-access example below separately tests what the child can retrieve.
...
The distinction is between reusing an old answer, asking the service again, and rerunning against captured evidence. Caos gives each set of inputs an inspectable identity.
...
Saving an output and showing it to the monitor are separate steps. Caos lets the monitor fetch the full result independently of the display. The next experiment follows an effect that disappears from the final snapshot.
...
The operator creates these history commits and retains both versions in the bundle. The useful comparison is against the history tip saved earlier: following a mutable branch name alone loses the evidence. An effect created and removed inside a single job would need finer-grained recording.
...
Deduplication lives at the service that applies the effect. A fresh Caos request is a new computation, even when it repeats the same intent. Our service keeps its ledger in memory and handles one request at a time; preserving that behavior across restarts would require storing the intent and effect atomically.
...
A narrow input tree describes what the child was handed. Read permissions determine what else it can fetch. Enforcing those permissions requires controlling the direct backend route as well as the gateway.
