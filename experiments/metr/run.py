...
def main():
    ...
    p.add_argument('--only', choices=['spoof', 'cache', 'erasure', 'all'], default='all')
    a = p.parse_args()
    ...
    for slug, case in config['cases'].items():
        ...
        shutil.copytree(HERE/'fixtures'/slug, fixture)
        shutil.copyfile(HERE/'fixtures/common/README.md', fixture/'STUDY.md')
        ...
        for stage in ('setup', 'attack', 'review'):
            ...
            call([sys.executable, REPO/'trajectories/export.py', '--server', a.server, '--head', head, '--id', row['id'], '--title', case['title'], '--description', 'An explicitly prompted adversarial study with real model responses and tool executions. See the experiment report for measured results.', '--runtime', manifest['runtime'], '--output', root/'data', '--forbid-file', key], root)
